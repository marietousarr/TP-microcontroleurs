...
// déclaration led et bouton poussoir 
...
do
{
	if ((BROCHE_BP -> IDR & (0x1 << PORT_BP))) // verifie si idr a reçu est modifié (bouton appuyé)
	{
		...
		BROCHE_LD -> ODR |= (0x1<< PORT_LD); // vient décocher eteint la led
	}
...
do
{
	if ((MyGPIO_Read(broche_bp.GPIO, 0x0D)==1)) // verifie si idr a reçu est modifié (bouton appuyé)
	{
